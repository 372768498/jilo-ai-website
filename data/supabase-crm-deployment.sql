...
CREATE INDEX IF NOT EXISTS idx_clients_manager ON clients(account_manager);
CREATE INDEX IF NOT EXISTS idx_clients_created_at ON clients(created_at);

CREATE INDEX IF NOT EXISTS idx_orders_client_id ON orders(client_id);
CREATE INDEX IF NOT EXISTS idx_orders_status ON orders(order_status);
CREATE INDEX IF NOT EXISTS idx_orders_date ON orders(order_date);
CREATE INDEX IF NOT EXISTS idx_orders_value ON orders(order_value);
...
BEGIN
    ...
    AND routine_name IN ('update_updated_at_column', 'get_client_summary');
    
    -- 检查视图数量
    SELECT COUNT(*) INTO view_count
    FROM information_schema.views 
    WHERE table_schema = 'public'
    AND table_name IN ('client_overview', 'sales_performance');
    
    -- 输出验证结果
    RAISE NOTICE 'CRM系统数据库部署验证:';
    RAISE NOTICE '表数量: % (预期: 7)', table_count;
    RAISE NOTICE '索引数量: % (预期: 20+)', index_count;
    RAISE NOTICE '函数数量: % (预期: 2)', function_count;
    RAISE NOTICE '视图数量: % (预期: 2)', view_count;
    
    IF table_count = 7 AND index_count >= 20 AND function_count = 2 AND view_count = 2 THEN
        RAISE NOTICE '✅ CRM系统数据库部署成功！';
    ELSE
        RAISE NOTICE '❌ CRM系统数据库部署可能存在问题，请检查。';
    END IF;
END $$;
